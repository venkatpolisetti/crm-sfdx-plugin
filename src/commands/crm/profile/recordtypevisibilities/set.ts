...
export default class set extends SfdxCommand {
	...
	protected static flagsConfig = {
		...
		recordtypes: flags.string({ char: 'r', required:true, description: messages.getMessage('recordtypesFlagDescription') }),
			...
			if (this.flags.verbose) {
				...
				profileMetadata.forEach(p => {
					p.recordTypeVisibilities.forEach(r => {
						profileRecTypeArray.push({profile:standardProfiles.find(s => s.metaName == p.fullName).name, recordtype:r.recordType, visible:r.visible, default:r.default});
					});
				});
